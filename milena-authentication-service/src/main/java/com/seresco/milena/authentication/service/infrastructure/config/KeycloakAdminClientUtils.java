...
public class KeycloakAdminClientUtils {
    ...
    /**
     * Loads the domain configuration from system property.
     * 
    ...
    public static KeycloakAdminClientConfig loadConfig(KeycloakPropertyReader keycloakPropertyReader) {
        ...
        try {
            ...
            log.error("Error: Loading domain admin configuration => {}", e.getMessage());
        }

        KeycloakAdminClientConfig config = builder.build();
        log.debug("Found domain configuration: {}", config);
     ...
     * contains sub roles.
     * 
     * @param keycloak                  domain instance
     * @param keycloakAdminClientConfig domain configuration
